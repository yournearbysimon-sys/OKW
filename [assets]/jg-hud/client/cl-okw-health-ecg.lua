...
--- Sprint stamina as 0–100 (GET_PLAYER_SPRINT_STAMINA_REMAINING typically max ~40 when full).
local function sprintStaminaPercent(playerId, staminaMax)
    ...
    return math.max(0.0, math.min(100.0, (rem / staminaMax) * 100.0))
end
...
CreateThread(function()
    ...
    while true do
        ...
        local st = sprintStaminaPercent(pid, stMax)
...
CreateThread(function()
    ...
    while true do
        ...
        if ped and ped ~= 0 then
            if useStam then
                pct = sprintStaminaPercent(pid, tonumber(ecg.staminaMax) or 40.0)
            else
