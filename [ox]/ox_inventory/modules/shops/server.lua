...
lib.callback.register('ox_inventory:openShop', function(source, data)
	...
	return { id = source, label = playerInv.label, type = playerInv.type, slots = playerInv.slots, weight = playerInv.weight, maxWeight = playerInv.maxWeight }, shop
end)
...
local function canAffordItem(inv, currency, price)
	...
	if currency ~= 'bank' then
		...
		canAfford = (price >= 0 and server.getBankMoney(inv) >= price)
	end
...
local function removeCurrency(inv, currency, price)
	if currency ~= 'bank' then
		...
		server.removeBankMoney(inv, price)
	end
end
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local message = locale('purchased_for', count, metadata?.label or fromItem.label, (currency == 'money' and locale('$') or math.groupdigits(price)), (currency == 'money' and math.groupdigits(price) or currency ~= 'bank' and ' '..Items(currency).label or 'bank'))
