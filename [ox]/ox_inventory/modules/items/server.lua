...
local function isConfiguredArmorItem(itemName)
	itemName = itemName and itemName:lower()
	return itemName and utilityConfig.armorItems and utilityConfig.armorItems[itemName] ~= nil
end

AddEventHandler('ox_inventory:usedItem', function(invId, itemName, slot, metadata, source)
	if itemName == 'armor_repair_kit' or itemName == 'armor_plates' then
		local playerInv = Inventory(invId)
		local armourInv = Inventory(source)
		if not playerInv then return end

		local items = armourInv.items
		local armourSlot
		local armourItem

		for _, item in pairs(items) do
			if isConfiguredArmorItem(item.name) then
				armourSlot = item.slot
				armourItem = item
				break
			end
		end

		if not armourItem then
			TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = 'You are not wearing any armor to repair!' })
			return
		end

		local durability = armourItem.metadata.durability or 100
		if durability >= 100 then
			TriggerClientEvent('ox_lib:notify', source, { type = 'error', description = 'Your armor is already in perfect condition!' })
			return
		end

		local repairValue = utilityConfig.armorRepairItems and utilityConfig.armorRepairItems[itemName] or 20

		local progress = lib.callback.await('ox_inventory:repairArmorProgress', source, tonumber(utilityConfig.repairProgressDuration) or 5000)

		if not progress then return end

		local newDurability = math.min(100, durability + repairValue)
		armourItem.metadata.durability = newDurability
		Inventory.SetMetadata(playerInv.id, armourSlot, armourItem.metadata)

		local armorDef = Items(armourItem.name)
		local divider = armorDef and armorDef.divider or 1

		if not armorDef.divider and utilityConfig.armorItems[armourItem.name] and utilityConfig.armorItems[armourItem.name].value then
			divider = 100 / utilityConfig.armorItems[armourItem.name].value
		end

		local newPedArmour = math.floor(newDurability / divider)
		local ped = GetPlayerPed(source)
		SetPedArmour(ped, newPedArmour)
		Inventory.RemoveItem(invId, itemName, 1, nil, slot)
		if itemName == 'armor_repair_kit' then
			TriggerClientEvent('ox_lib:notify', source, { type = 'success', description = 'You repaired your armor.' })
		else
			TriggerClientEvent('ox_lib:notify', source, { type = 'success', description = 'You reinforced your armor.' })
		end
	end
end)
