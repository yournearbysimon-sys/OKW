...
for i = 1, #utilitySlotIds do
	utilitySlotLookup[utilitySlotIds[i]] = true
end

local function isConfiguredUtilitySlot(slot)
	return utilitySlotLookup[slot] == true
end

local function utilitySlotAcceptsItem(slot, itemName)
	...
	for i = 1, #allowed do
		if allowed[i]:lower() == itemName:lower() then
			return true
		end
	end
...
local function isConfiguredBackpackSlot(slot, itemName)
	return slot == getConfiguredBackpackSlotId()
		and utilityConfig.backpackItems
		and utilityConfig.backpackItems[itemName] ~= nil
end

local function validateBackpackRemoval(inv, slotData)
	if utilityConfig.lockBackpackRemovalWithItems == false then
		return true
	end

	if not inv or not inv.player or not slotData then
		return true
	end

	if not isConfiguredBackpackSlot(slotData.slot, slotData.name) then
		return true
	end

	if not slotData.metadata or not slotData.metadata.container or not slotData.metadata.size then
		return true
	end

	local container = Inventory.GetContainerFromSlot(inv, slotData.slot)

	if container and container.items and next(container.items) ~= nil then
...
function Inventory.SwapSlots(fromInventory, toInventory, slot1, slot2)
    ...
    if fromInventory.type == 'drop' then
        Inventory.Drops[fromInventory.id].items = fromInventory.items
        TriggerClientEvent('ox_inventory:updateDrop', -1, fromInventory.id, fromInventory.items)
    end
    if toInventory.type == 'drop' then
        Inventory.Drops[toInventory.id].items = toInventory.items
        TriggerClientEvent('ox_inventory:updateDrop', -1, toInventory.id, toInventory.items)
    end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if not toSlot then
		...
		for i = 1, inv.slots do
			...
			elseif not item.stack and not slotData then
				local canUseSlot = true
				if inv.player then
					if isConfiguredUtilitySlot(i) then
						canUseSlot = false
						canUseSlot = utilitySlotAcceptsItem(i, item.name)
					end
				end
			...
			elseif not toSlot and not slotData and not inv.player then
				toSlot = i
			elseif not toSlot and not slotData and inv.player then
				if not isConfiguredUtilitySlot(i) then
					toSlot = i
				elseif utilitySlotAcceptsItem(i, item.name) then
					toSlot = i
				end
			end
		end
    ...
    local previousSlotCounts

    if inv.type == 'drop' then
        if toSlotType == 'number' then
            previousSlotCounts = inv.items[toSlot] and inv.items[toSlot].count or 0
        elseif toSlotType == 'table' then
            previousSlotCounts = {}

            for i = 1, #toSlot do
                local slotIndex = toSlot[i].slot
                previousSlotCounts[slotIndex] = inv.items[slotIndex] and inv.items[slotIndex].count or 0
            end
        end
    end
    ...
    if success and inv.type == 'drop' then
        local itemsToAdd

        if toSlotType == 'number' then
            local slotItem = response

            if slotItem then
                local prev = type(previousSlotCounts) == 'number' and previousSlotCounts or 0
                local delta = slotItem.count - prev
        ...
        elseif toSlotType == 'table' and type(response) == 'table' then
            local idx = 0
            itemsToAdd = {}

            for i = 1, #response do
                local slotItem = response[i]

                if slotItem then
                    local prev = type(previousSlotCounts) == 'table' and previousSlotCounts[slotItem.slot] or 0
                    local delta = slotItem.count - prev
        ...
        Inventory.Drops[inv.id].items = inv.items
        TriggerClientEvent('ox_inventory:updateDrop', -1, inv.id, inv.items)

        local dropData = setupDropEntry(inv.id)

        if itemsToAdd and dropData then
            dropData.hasPropObjects = dropPropConfig.enabled
            AddDropProps(inv.id, itemsToAdd, inv.coords)
        end

		reconcileDropProps(inv.id)
    end
...
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal, strict)
	...
	local dropRemovals = inv.type == 'drop' and {} or nil
	...
	if slot and itemSlots[slot] then
		local canRemoveBackpack, backpackError = validateBackpackRemoval(inv, inv.items[slot])

		if canRemoveBackpack == false then
			return false, backpackError
		end
		...
		if dropRemovals then
			...
			if slotItem then
				dropRemovals[#dropRemovals + 1] = {
					name = slotItem.name,
					count = count,
					slot = slotItem.slot or slot
				}
			end
		end
	...
	elseif itemSlots and totalCount > 0 then
		for k, v in pairs(itemSlots) do
			if removed < total then
				local canRemoveBackpack, backpackError = validateBackpackRemoval(inv, inv.items[k])

				if canRemoveBackpack == false then
					return false, backpackError
				end

				local slotItem = inv.items[k]
				if v == count then
					TriggerClientEvent('ox_inventory:itemNotify', inv.id, { inv.items[k], 'ui_removed', v })

					if dropRemovals and slotItem then
						dropRemovals[#dropRemovals + 1] = {
							name = slotItem.name,
							count = v,
							slot = slotItem.slot or k
						}
					end
	...
	if removed > 0 then
		...
		if inv.type == 'drop' then
			...
			if dropRemovals then
				for i = 1, #dropRemovals do
					local entry = dropRemovals[i]

					if entry and entry.name and entry.count and entry.count > 0 then
						RemoveDropProps(inv.id, entry.name, entry.slot, entry.count)
					end
				end
			end
...
local dropPropConfig = {
	-- default to enabled; can still be disabled via convar
	enabled = GetConvarInt('inventory:dropprops', 1) == 1,
	oneItemOneProp = GetConvarInt('inventory:oneitemoneprop', 1) == 1,
	defaultModel = "hei_prop_hei_paper_bag"
}
...
local function normaliseModel(model)
	...
	return dropPropConfig.defaultModel
end

function setupDropEntry(dropId)
	local drop = Inventory.Drops[dropId]

	if drop then
		drop.itemProps = drop.itemProps or {}
		drop.propLookup = drop.propLookup or {}
		drop.netLookup = drop.netLookup or {}
		drop.oneItemOneProp = drop.oneItemOneProp ~= nil and drop.oneItemOneProp or dropPropConfig.oneItemOneProp
		drop.hasPropObjects = dropPropConfig.enabled and drop.hasPropObjects ~= false or false
	end

	return drop
end

local function GenerateUniqueItemId(dropId, itemName, slotRef, index)
	return ('%s_%s_%s_%d_%d'):format(dropId, itemName or 'unknown', slotRef or 'slot', index or 1, math.random(10000, 99999))
end

local function CalculateItemPosition(baseCoords, index, totalItems, baseZ)
	if index == 1 then
		return vector3(baseCoords.x, baseCoords.y, baseZ or baseCoords.z)
	end

	local zoneType = math.random(1, 3)
	local radius
	...
	local scatterX = (math.random() - 0.5) * 0.25
	local scatterY = (math.random() - 0.5) * 0.25

	return vector3(
		baseCoords.x + (radius * math.cos(radians)) + scatterX,
		baseCoords.y + (radius * math.sin(radians)) + scatterY,
		baseZ or baseCoords.z
	)
end

function GetDropItemModel(itemName, src)
	local src = source or src
	...
	if src and isWeapon then
		local resolvedWeaponModel = lib.callback.await('ox_inventory:resolveModelOnClient', src, itemName)
		if resolvedWeaponModel and resolvedWeaponModel ~= 0 then
			return resolvedWeaponModel
		end
	end

    local item = Items(itemName)
    if item then
        if type(item.client) == 'table' then
            local clientData = item.client

            if clientData.dropModel then
                return normaliseModel(clientData.dropModel)
            end

            local prop = clientData.prop
            if type(prop) == 'table' and prop.model then
                return normaliseModel(prop.model)
            elseif type(prop) == 'table' and prop.prop then
                return normaliseModel(prop.prop)
            elseif prop then
                return normaliseModel(prop)
            end

            if clientData.prop then
                return normaliseModel(clientData.prop)
            end
        elseif item.client then
            return normaliseModel(item.client)
        end

        local itemProp = item.prop
        if type(itemProp) == 'table' and itemProp.model then
            return normaliseModel(itemProp.model)
        elseif type(itemProp) == 'table' and itemProp.prop then
            return normaliseModel(itemProp.prop)
        elseif itemProp then
            return normaliseModel(itemProp)
        end
    end
...
function AddDropProps(dropId, items, baseCoords, targetSource, modelOverride)
	if not dropPropConfig.enabled or not items then return end

	local drop = setupDropEntry(dropId)

	if not drop then return end

	local coords = baseCoords or drop.coords

	if not coords then return end

	local baseZ = drop.baseZ or coords.z
	local currentCount = #drop.itemProps
	...
	for i = 1, #items do
		...
		if itemName then
			local amount = itemData.count or itemData.amount or 1
			local slot = itemData.slot or itemData.slotRef
			local model = modelOverride or GetDropItemModel(itemName, targetSource)
			local createCount = drop.oneItemOneProp and 1 or amount

			for index = 1, createCount do
				currentCount += 1

				local position = CalculateItemPosition(coords, currentCount, currentCount + 10, baseZ)
				local uniqueId = GenerateUniqueItemId(dropId, itemName, slot, index)
				local entry = {
					uniqueId = uniqueId,
					itemName = itemName,
					slot = slot,
				...
				drop.itemProps[#drop.itemProps + 1] = entry
				drop.propLookup[uniqueId] = entry

				TriggerClientEvent('ox_inventory:createDropProp', targetSource or -1, {
					prop = model,
					coords = position,
					dropId = dropId,
					uniqueId = uniqueId,
	...
	drop.hasPropObjects = drop.hasPropObjects or #drop.itemProps > 0
end

function RemoveDropProps(dropId, itemName, slot, amount)
	if not dropPropConfig.enabled then return end

	local drop = Inventory.Drops[dropId]

	if not drop or not drop.itemProps then return end

	local required = drop.oneItemOneProp and 1 or (amount or 1)
	local removed = 0

	for index = #drop.itemProps, 1, -1 do
		local entry = drop.itemProps[index]

		if entry.itemName == itemName and (slot == nil or entry.slot == slot) then
			local netId = entry.netId

			if netId then
				TriggerClientEvent('ox_inventory:removeDropProp', -1, netId, entry.uniqueId)
				if drop.netLookup then
					drop.netLookup[netId] = nil
				end
			else
				TriggerClientEvent('ox_inventory:removeDropProp', -1, nil, entry.uniqueId)
			end

			drop.propLookup[entry.uniqueId] = nil
			table.remove(drop.itemProps, index)
			removed += 1
	...
	if #drop.itemProps == 0 then
		drop.baseZ = nil
		drop.hasPropObjects = false
	end
end

function ClearDropProps(dropId)
	if not dropPropConfig.enabled then return end
...
local function reconcileDropProps(dropId)
	if not dropPropConfig.enabled then return end
	...
	for index = #drop.itemProps, 1, -1 do
		...
		if not item or item.name ~= entry.itemName then
			local netId = entry.netId
	...
	drop.hasPropObjects = drop.itemProps and #drop.itemProps > 0 or false
end

RegisterNetEvent('ox_inventory:registerDropProp', function(uniqueId, netId, finalCoords)
	if not uniqueId or not netId then return end

	for dropId, drop in pairs(Inventory.Drops) do
		if drop.propLookup then
			local entry = drop.propLookup[uniqueId]

			if entry then
				local previousNetId = entry.netId
				entry.netId = netId
				drop.hasPropObjects = true

				if previousNetId and drop.netLookup then
					drop.netLookup[previousNetId] = nil

					if previousNetId ~= netId then
						TriggerClientEvent('ox_inventory:removeDropProp', -1, previousNetId, uniqueId)
					end
				end

				if finalCoords then
					entry.coords = finalCoords

					if not drop.baseZ then
						drop.baseZ = finalCoords.z
					end
				end

				drop.netLookup = drop.netLookup or {}
				drop.netLookup[netId] = uniqueId
				return
			end
		end
	end
...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	Inventory.Drops[dropId] = {
		...
		itemProps = {},
		hasPropObjects = false
	}

	local dropData = setupDropEntry(dropId)

	if dropData then
		dropData.hasPropObjects = dropPropConfig.enabled and inventory.items and next(inventory.items) ~= nil or false
	end

	TriggerClientEvent('ox_inventory:createDrop', -1, dropId, Inventory.Drops[dropId])

	if not model and dropPropConfig.enabled and dropData and inventory.items and next(inventory.items) ~= nil then
		local propItems, count = {}, 0
		...
		if count > 0 then
			print("AddDropProps", dropId, propItems, inventory.coords, source, model)
			AddDropProps(dropId, propItems, inventory.coords, source, model)
		end
	end
...
exports('CreateDropFromPlayer', function(playerId)
	...
	if dropPropConfig.enabled and dropData and inventory.items and next(inventory.items) ~= nil then
		local propItems, count = {}, 0
		...
		if count > 0 then
			AddDropProps(dropId, propItems, inventory.coords)
		end
	end
...
---@field toType string
---@field fromInventoryId string?
---@field toInventoryId string?
...
local function dropItem(source, fromInventory, fromData, data)
	...
	-- resolve player inventory for actor-specific context (open state, logging, etc)
	...
	if dropPropConfig.enabled then
		AddDropProps(dropId, {
			{
				name = toData.name,
				count = data.count,
	...
	if server.syncInventory then
		-- sync the source inventory (which changed)
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	local function getInventory(type, inventoryId)
		if type == 'player' then return playerInventory end
		if type == 'backpack' then
			local backpackSlotId = getConfiguredBackpackSlotId()

			local function resolveBackpack(ownerInventory)
				if not ownerInventory or not utilityConfig.backpackItems then return end

				local item = ownerInventory.items[backpackSlotId]

				if item and utilityConfig.backpackItems[item.name] and item.metadata.container then
					return Inventory(item.metadata.container)
				end
			end

			local playerBackpack = resolveBackpack(playerInventory)

			if inventoryId then
				if playerBackpack and playerBackpack.id == inventoryId then
				...
				if openedInventory and openedInventory.player and openedInventory ~= playerInventory then
					local targetBackpack = resolveBackpack(openedInventory)

					if targetBackpack and targetBackpack.id == inventoryId then
						return targetBackpack
					end
				end
		...
		if type == 'crafting_storage' then
			if playerInventory.craftingStorageId then
				local storageInv = Inventory(playerInventory.craftingStorageId)
				return storageInv
			end
			return
		end
	...
	local toInventory = getInventory(data.toType, data.toInventoryId)
	local fromInventory = getInventory(data.fromType, data.fromInventoryId)
	...
	local fromInitialCount = fromData and fromData.count or 0
	local fromInitialName = fromData and fromData.name or nil
	local toInitialCount = toData and toData.count or 0
	local toInitialName = toData and toData.name or nil
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		local canRemoveBackpack, backpackError = validateBackpackRemoval(fromInventory, fromData)
		...
		if fromData then
			...
			if fromInventory.type == 'drop' then
				local finalFromItem = fromInventory.items[data.fromSlot]
				local finalFromCount = finalFromItem and finalFromItem.count or 0
				local fromDiff = finalFromCount - fromInitialCount

				if fromDiff > 0 and finalFromItem then
					AddDropProps(fromInventory.id, {
						{
							name = finalFromItem.name,
							count = fromDiff,
							slot = data.fromSlot
						}
					}, fromInventory.coords)
				elseif fromDiff < 0 and fromInitialName then
					RemoveDropProps(fromInventory.id, fromInitialName, data.fromSlot, -fromDiff)
				end

				Inventory.Drops[fromInventory.id].items = fromInventory.items
				TriggerClientEvent('ox_inventory:updateDrop', -1, fromInventory.id, fromInventory.items)
				reconcileDropProps(fromInventory.id)
			end

			if toInventory.type == 'drop' then
				local finalToItem = toInventory.items[data.toSlot]
				local finalToCount = finalToItem and finalToItem.count or 0
				local toDiff = finalToCount - toInitialCount

				if toDiff > 0 and finalToItem then
					AddDropProps(toInventory.id, {
						{
							name = finalToItem.name,
							count = toDiff,
							slot = data.toSlot
						}
					}, toInventory.coords)
				elseif toDiff < 0 and toInitialName then
					RemoveDropProps(toInventory.id, toInitialName, data.toSlot, -toDiff)
				end

				Inventory.Drops[toInventory.id].items = toInventory.items
				TriggerClientEvent('ox_inventory:updateDrop', -1, toInventory.id, toInventory.items)
				reconcileDropProps(toInventory.id)
			end
...
local function saveInventories(clearInventories)
    ...
    for _, inv in pairs(Inventories) do
        if not inv.open and not inv.player and inv.type ~= 'temp' then
...
local giveItemDistance = 4.0

local function giveItem(playerId, slot, target, count, fromInventoryId)
	local playerInventory = Inventory(playerId)
	local fromInventory = fromInventoryId and Inventory(fromInventoryId) or playerInventory
	local toInventory = Inventory(target)

	if not playerInventory or not fromInventory or not toInventory?.player then
	...
	if not data then
		return { 'item_not_enough', 'item' }
	end

	local item = Items(data.name)

	if not item or data.count < count then
		return { 'item_not_enough', data.label or data.name or 'item' }
	end

	if #(GetEntityCoords(playerInventory.player.ped) - GetEntityCoords(toInventory.player.ped)) > giveItemDistance then
		return { 'nobody_nearby' }
	end

	if not Inventory.CanCarryItem(toInventory, item, count, data.metadata) then
		return { 'cannot_carry_other' }
	end
...
--- groups: { ['police'] = 0 }
--- ```
local function registerStash(name, label, slots, maxWeight, owner, groups, coords, distance)
	...
	if curStash then
		...
		for _, stash in pairs(Inventories) do
			if stash.type == 'stash' and stash.dbId == name then
				...
				stash.distance = distance or stash.distance
			end
		end
	end
...
local function removeStash(name)
	if type(name) ~= 'string' then return end

	RegisteredStashes[name] = nil
	Inventory.Remove(name)
end

exports('RemoveStash', removeStash)
