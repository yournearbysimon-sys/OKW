...
export const onDrop = (source: DragSource, target?: DropTarget, splitting?: boolean) => {
  ...
  let { sourceInventory, targetInventory } = getTargetInventory(
    state,
    source.inventory,
    target?.inventory,
    source.inventoryId,
  ...
  const targetSlot = target
    ...
    : findAvailableSlot(sourceSlot, sourceData, targetInventory.items, splitting, targetInventory.type);
  ...
  const count =
    state.splitModifierPressed && sourceSlot.count > 1 && sourceInventory.type !== 'shop'
