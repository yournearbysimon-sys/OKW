...
interface StashPermissionsData {
  stashId: string;
  stashLabel?: string;
  owner: {
    name: string;
...
interface StashPermissionsProps {
  embedded?: boolean;
  active?: boolean;
  stashId?: string;
  onClose?: () => void;
  initialTab?: StashPermissionsTab;
}

type StashPermissionsTab = 'roles' | 'settings' | 'members';
...
const StashPermissions: React.FC<StashPermissionsProps> = ({
  embedded = false,
  active = true,
  stashId,
  onClose,
  ...
  const [data, setData] = useState<StashPermissionsData | null>(null);
  const [selectedRole, setSelectedRole] = useState<number | null>(null);
  const [activeTab, setActiveTab] = useState<StashPermissionsTab>('roles');
  const [memberPickerOpen, setMemberPickerOpen] = useState(false);
  const [transferPickerOpen, setTransferPickerOpen] = useState(false);
  const [memberRolePicker, setMemberRolePicker] = useState<string | null>(null);
  const [editingRole, setEditingRole] = useState<Partial<StashRole> | null>(null);
  ...
  useNuiEvent<StashPermissionsData>('openStashPermissions', (payload) => {
  ...
  useEffect(() => {
    ...
    if (!stashId) {
      setLoadError(translate('stash_permissions_not_found', 'Stash not found'));
      return;
    }

    if (data?.stashId === stashId) return;

    setLoading(true);
    setLoadError(null);

    fetchNui<{ ok?: boolean; error?: string }>('stashPermissions:open', { stashId })
      .then((response) => {
        if (!response?.ok) {
          setLoading(false);
  ...
  }, [embedded, active, stashId, data?.stashId]);
  ...
  }, [activeTab, stashId, selectedRole]);

  useEffect(() => {
    if (!memberPickerOpen && !transferPickerOpen && !memberRolePicker) return;
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!transferPickerRef.current?.contains(event.target as Node)) {
        setTransferPickerOpen(false);
      }
  ...
  }, [memberPickerOpen, transferPickerOpen, memberRolePicker]);
  ...
  const handleClose = () => {
    ...
    setLoadError(null);
    setIsSubmitting(false);
    fetchNui('stashPermissions:close');
    onClose?.();
  };

  const handleUpdateRole = (role: Partial<StashRole>) => {
    if (!role.id) return;

    fetchNui('stashPermissions:updateRole', {
  ...
  const handleDeleteRole = (roleId: number) => {
    setConfirmation({
      ...
      onConfirm: () => {
        fetchNui('stashPermissions:deleteRole', {
          stashId: data?.stashId,
          roleId,
        });
        setConfirmation(null);
  ...
  const handleSetMemberRole = (target: string | number, roleId: number | '') => {
    fetchNui('stashPermissions:setMemberRole', {
      stashId: data?.stashId,
      target,
  ...
  const transferTargets = useMemo(
    () => (data?.onlinePlayers || []).filter((player) => player.identifier !== data?.owner?.identifier),
    [data?.onlinePlayers, data?.owner?.identifier]
  );
  ...
  const permissionItems = [
    {
      key: 'canUse',
      label: t('stash_permissions_use_stash', 'Use Stash'),
      desc: t('stash_permissions_use_stash_desc', 'Can open and use the stash'),
    },
    {
      key: 'canMove',
      label: t('stash_permissions_move_stash', 'Move Stash'),
      desc: t('stash_permissions_move_stash_desc', 'Can reposition the stash'),
    },
    {
      key: 'canPack',
      label: t('stash_permissions_pack_stash', 'Pack Stash'),
      desc: t('stash_permissions_pack_stash_desc', 'Can pack the stash item back up'),
    },
    {
      key: 'canManage',
      label: t('stash_permissions_manage_stash', 'Manage Stash'),
      desc: t('stash_permissions_manage_stash_desc', 'Can edit roles and members'),
    },
  ] as const;
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col leading-tight mt-0.5">
          <p className="text-white font-bold text-xl tracking-wide uppercase">
            {t('stash_permissions_title', 'Stash Permissions')}
          </p>
    ...
    <div className="mb-3 flex items-center gap-2 rounded-lg border border-[rgba(135,135,135,0.18)] bg-[linear-gradient(135deg,rgba(23,23,23,0.45)_0%,rgba(23,23,23,0.08)_100%)] p-1">
      {[
        ...
        return (
          <button
            key={tab.key}
            onClick={() => !isDisabled && setActiveTab(tab.key as StashPermissionsTab)}
            disabled={isDisabled}
  ...
  const renderRoleOverview = () => {
    if (!data) {
      return renderEmptyState(
        'inventory_2',
        t('stash_permissions_empty_title', 'No Stash Selected'),
        t('stash_permissions_empty_desc', 'Open a placed stash to manage its permissions.')
      );
    }
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[11px] leading-4 text-[#A8A8A8]">
              {t('stash_permissions_roles_desc', 'Select a role to manage stash access and members.')}
            </p>
          </div>
        ...
        {isOwner && (
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              ...
              className="flex h-[34px] items-center justify-center gap-2 rounded-md border border-dashed border-[rgba(135,135,135,0.2)] text-[10px] font-semibold uppercase tracking-[0.08em] text-white/60 transition-colors hover:border-[rgba(var(--color-primary-rgb),0.35)] hover:text-[var(--color-primary)]"
            >
            ...
            <div className="relative" ref={transferPickerRef}>
              {transferPickerOpen && (
                <div className="absolute top-[calc(100%+8px)] left-0 right-0 z-20 overflow-hidden rounded-md border border-[rgba(135,135,135,0.18)] bg-[linear-gradient(135deg,rgba(40,40,40,0.96)_0%,rgba(28,28,28,0.88)_100%)] shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
                  <div className="max-h-[170px] overflow-y-auto p-1">
                    {transferTargets.length === 0 ? (
                      <div className="px-3 py-3 text-[12px] text-white/45">
                        {t('permissions_no_nearby_players', 'No nearby players available')}
                      </div>
                    ) : (
                      transferTargets.map((player) => (
                        <button
                          ...
                          onClick={() => {
                            fetchNui('stashPermissions:transferOwnership', {
                              stashId: data.stashId,
              ...
              <button
                disabled={!data.canTransfer}
                onClick={() => data.canTransfer && setTransferPickerOpen((open) => !open)}
                className={`flex h-[34px] w-full items-center justify-center gap-2 rounded-md border text-[10px] font-semibold uppercase tracking-[0.08em] transition-colors ${
                  data.canTransfer
                    ? 'border-[rgba(135,135,135,0.18)] text-white/75 hover:border-[rgba(var(--color-primary-rgb),0.35)] hover:text-[var(--color-primary)]'
                    : 'cursor-not-allowed border-[rgba(135,135,135,0.12)] text-white/25'
                }`}
              >
                <span className="material-symbols-outlined text-[15px]">swap_horiz</span>
                <span>{t('permissions_transfer', 'Transfer')}</span>
              </button>
            </div>
          </div>
        )}
  ...
  const renderMainContent = () => {
    if (loading) {
      return renderEmptyState(
        ...
        t('stash_permissions_loading_desc', 'Fetching stash roles and member access.')
      );
    }
    ...
    if (!selectedRoleData) {
      return renderEmptyState(
        ...
        t('stash_permissions_select_role_desc', 'Open the Roles tab and choose a role to manage stash access.')
      );
    }
    ...
    return (
      <div className="flex h-full flex-col overflow-hidden">
        {activeTab === 'settings' ? (
          <>
            {renderSelectedRoleHeader(t('stash_permissions_role_access_desc', 'Configure permissions and stash access for this role.'), true)}
          ...
          <>
            {renderSelectedRoleHeader(t('stash_permissions_assigned_members_desc', 'Manage assigned members for this role.'))}
              ...
              <div className="mt-3 flex flex-1 flex-col gap-2 overflow-y-auto pr-1">
                <div className="rounded-md border border-[rgba(var(--color-primary-rgb),0.18)] bg-[rgba(var(--color-primary-rgb),0.05)] px-2.5 py-2">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/40">
                    {t('stash_permissions_owner_card', 'Stash Owner')}
                  </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <button
                ...
                onClick={() => {
                  ...
                  if (!editingRole.id) {
                    fetchNui('stashPermissions:createRole', {
                      stashId: data?.stashId,
...
export default StashPermissions;
