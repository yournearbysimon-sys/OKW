...
RegisterNUICallback('radialBack', function(_, cb)
    ...
    -- If menu was closed during transition, don't open the submen
    if not isOpen then return end

    SendNUIMessage({
        action = 'openRadialMenu',
        data = {
            items = Utils.sanitizeItems(menuItems),
...
local function WaitForUtils()
    local maxAttempts = 100
    local attempts = 0

    while not Utils and attempts < maxAttempts do
        Wait(100)
        attempts = attempts + 1
    end
...
Utils.addKeybind({
    ...
    defaultKey = GetConvar('prism:radialOpenKey', 'z'),
    onPressed = function()
    ...
    onReleased = GetConvar('prism:radialOpenMode', "press") == 'hold' and HideRadial or nil
})
